import numpy as np
import sys
import torch
import torch.autograd as autograd
import torch.nn as nn
import torch.optim as optim
from torch.utils.data import DataLoader, TensorDataset


class ChebNet_arch(nn.Module):
    def __init__(self, X_means, X_stds, y_mean, y_std, phi_layer=None, K_layer =None, degree=15, K='mean'):
        super(ChebNet_arch, self).__init__()
        self.X_means = X_means
        self.X_stds = X_stds
        self.y_mean = y_mean
        self.y_std = y_std
        self.n_in = X_means.shape[1]
        self.y_dim = 1 if len(y_mean.shape) == 1 else y_mean.shape[1]
        self.degree = degree
        self.K = K
        
        self.phi_layer = phi_layer if phi_layer is not None else [self.n_in, 200, 200, self.degree]
        self.K_layer = K_layer if K_layer is not None else [self.n_in, 200, 200, 1]
        
        phi_layers = []
        for i in range(len(self.phi_layer) - 2):
            phi_layers.append(
                nn.Sequential(
                    nn.Linear(self.phi_layer[i], self.phi_layer[i+1]),
                    nn.ReLU()))
            
        phi_layers.append(nn.Linear(self.phi_layer[-2], self.degree))
        phi_layers.append(nn.Softplus())
        
        self.phi_net = nn.Sequential(*phi_layers)
                
        K_layers = []
        for i in range(len(self.K_layer ) - 2):
            K_layers.append(nn.Sequential(
                nn.Linear(self.K_layer[i], self.K_layer[i+1]),
                nn.ReLU()))
            
        self.K_net = nn.Sequential(*K_layers, nn.Linear(self.K_layer[-2], 1))
        

    def _cheb_cs(self, x):
        K_output = self.K_net(x).view(-1)  # [batch]
        phi_out = self.phi_net(x)          # [batch, degree]
        # DCT-II
        N = phi_out.shape[1]
        v = torch.cat([phi_out, phi_out.flip([1])], dim=1)
        Vc = torch.fft.fft(v, dim=1)
        c_js = Vc[:, :N].real / N
        c_js[:, 0] = c_js[:, 0] / 2
        batch = c_js.shape[0]
        C_is = torch.zeros(batch, self.degree, device=c_js.device)
        for i in range(1, self.degree-1):
            C_is[:, i] = (c_js[:, i-1] - c_js[:, i+1]) / (4 * i)
        C_is[:, self.degree-1] = 0.25 * c_js[:, self.degree-2] / (self.degree-1)
        if self.K == 'q0':
            sign = torch.tensor([(-1)**k for k in range(1, self.degree)], device=c_js.device).float()
            C0 = 2 * K_output - 2 * torch.sum(C_is[:, 1:] * sign, dim=1)
            C_is[:, 0] = C0
        elif self.K == 'Mean':
            idx = torch.arange(1, self.degree, device=c_js.device)
            mask = (idx % 2 == 1)
            denom = (idx[mask]**2 - 4).float()
            C0 = 2 * K_output - 2 * torch.sum(C_is[:, 1:][:, mask] / denom, dim=1)
            C_is[:, 0] = C0
        return C_is

    def _eval_cheb(self, tau, cs):
        # tau: [batch, 1]
        # cs: [batch, degree]
        if tau.dim() == 1:
            tau = tau.unsqueeze(1)
        assert tau.shape[1] == 1, "tau must be of shape [batch, 1]"
        batch = tau.shape[0]
        xs = (tau * 2) - 1  # [batch, 1]
        d1 = torch.zeros(batch, 1, device=tau.device)
        d2 = torch.zeros(batch, 1, device=tau.device)
        for j in range(self.degree - 1, 0, -1):
            temp = d1.clone()
            d1 = 2 * xs * d1 - d2 + cs[:, j].unsqueeze(1)
            d2 = temp
        FF = xs * d1 - d2 + 0.5 * cs[:, 0].unsqueeze(1)
        return FF  # [batch, 1]

    def forward(self, x, tau):
        """
        x: [batch, input_dim]
        tau: [batch, 1]
        returns: [batch, 1] quantile predictions
        """
        if tau.dim() == 1:
            tau = tau.unsqueeze(1)
        assert tau.shape[1] == 1, "tau must be of shape [batch, 1]"
        cs = self._cheb_cs(x)  # [batch, degree]
        output = self._eval_cheb(tau, cs)  # [batch, 1]
        return output

    def predict(self, X, quantiles):
        self.eval()
        self.zero_grad()
        self.quantiles = quantiles
        tX = autograd.Variable(torch.FloatTensor((X - self.X_means) / self.X_stds), requires_grad=False)
        size = X.shape[0]
        preds = torch.zeros([size, len(self.quantiles)])
        for t in range(len(self.quantiles)):
            pred = self._eval_cheb(self.quantiles[t].repeat(size, 1).float(), self._cheb_cs(tX))
            preds[:, t] = pred.detach().squeeze()*self.y_std + self.y_mean
        return preds






class ChebNet:
    def __init__(self, quantiles):
        self.quantiles = quantiles
       
    def fit(self, X, y, phi_layer=None, K_layer =None, degree=15, K='mean',lr =0.1, epochs=100):
        
        if phi_layer is None:
            phi_layer = [X.shape[1], 200,200, degree]
        if K_layer is None:
            K_layer = [X.shape[1], 200, 200, 1]
        

        self.model = fit_quantiles(X, y, phi_layer=phi_layer, K_layer =K_layer, degree=degree, K=K,lr =lr, nepochs = epochs)

    def predict(self, X, quantiles):
         return self.model.predict(X, quantiles)
     
        

def fit_quantiles(X, y, phi_layer=None, K_layer =None, degree=15, K='mean',
                    nepochs=100, val_pct=0.25,
                    batch_size=None, target_batch_pct=0.01,
                    min_batch_size=20, max_batch_size=100,
                    verbose=False, lr=1e-1, weight_decay=0.0, patience=5,
                    init_model=None, splits=None, 
                    clip_gradients=False, clip_value=5,**kwargs):
 
    
    if batch_size is None:
        batch_size = min(X.shape[0], max(min_batch_size, min(max_batch_size, int(np.round(X.shape[0]*target_batch_pct)))))
        if verbose:
            print('Auto batch size chosen to be {}'.format(batch_size))

    # Standardize the features and response (helps with gradient propagation)
    Xmean = X.mean(axis=0, keepdims=True)
    Xstd = X.std(axis=0, keepdims=True)
    Xstd[Xstd == 0] = 1 # Handle constant features
    ymean, ystd = y.mean(axis=0, keepdims=True), y.std(axis=0, keepdims=True)
    tX = autograd.Variable(torch.FloatTensor((X - Xmean) / Xstd), requires_grad=False)
    tY = autograd.Variable(torch.FloatTensor((y - ymean) / ystd), requires_grad=False)
 
    # Create train/validate splits
    if splits is None:
        indices = np.arange(X.shape[0], dtype=int)
        np.random.shuffle(indices)
        train_cutoff = int(np.round(len(indices)*(1-val_pct)))
        train_indices = indices[:train_cutoff]
        validate_indices = indices[train_cutoff:]
    else:
        train_indices, validate_indices = splits
        
    train_dataset = TensorDataset(tX[train_indices], tY[train_indices])
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
    X_valid = tX[validate_indices]
    Y_valid = tY[validate_indices]
    u_valid = torch.rand(Y_valid.shape)
    #tquantiles = autograd.Variable(torch.FloatTensor(u.detach()), requires_grad=False)

    model = ChebNet_arch(Xmean, Xstd, ymean, ystd, phi_layer=phi_layer, K_layer =K_layer, degree=degree, K=K) if init_model is None else init_model

    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=1, gamma=0.5)

    # Track progress
    train_losses, val_losses, best_loss = np.zeros(nepochs), np.zeros(nepochs), None
    num_bad_epochs = 0

    if verbose:
        print('ymax and min:', tY.max(), tY.min())

    # Univariate quantile loss
  
    def quantile_loss(yhat, y_true, u):
        z = y_true - yhat
        return torch.max(u.view(1, -1) * z, (u.view(1, -1) - 1) * z)
    

    # Create the quantile loss function
    lossfn = quantile_loss
            

    # Train the model
    for epoch in range(nepochs):
        if verbose:
            print('\t\tEpoch {}'.format(epoch+1))
            sys.stdout.flush()
        train_loss = torch.Tensor([0])
       
        for batch_X, batch_Y in train_loader:
            batch_X = batch_X.float()
            batch_Y = batch_Y.float()
            batch_u = autograd.Variable(torch.rand(batch_Y.shape), requires_grad=False)
            model.train()
            model.zero_grad()
            
            batch_yhat = model(batch_X, batch_u)
            loss = lossfn(batch_yhat, batch_Y, batch_u).mean()
            loss.backward()
            
            if clip_gradients:
                torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=clip_value)
            
            optimizer.step()
            train_loss += loss.data
            
            if np.isnan(loss.data.numpy()):
                import warnings
                warnings.warn('NaNs encountered in training model.')
                break
            
            del batch_X, batch_Y, batch_u, loss
            
        validate_loss = torch.Tensor([0])

        with torch.no_grad():
            model.eval()
            model.zero_grad()
            yhat = model(X_valid, u_valid)
            loss = lossfn(yhat, Y_valid, u_valid)
            validate_loss = loss.sum().item()

        train_losses[epoch] = train_loss / float(len(train_indices))
        val_losses[epoch] = validate_loss / float(len(validate_indices))


        # Adjust the learning rate down if the validation performance is bad
        if num_bad_epochs > patience:
            if verbose:
                print('Decreasing learning rate to {}'.format(lr*0.5))
            scheduler.step()
            lr *= 0.5
            num_bad_epochs = 0


        # Check if we are currently have the best held-out log-likelihood
        if epoch == 0 or val_losses[epoch] <= best_loss:
            if verbose:
                print('\t\t\tSaving test set results.      <----- New high water mark on epoch {}'.format(epoch+1))
            # If so, use the current model on the test set
            best_loss = val_losses[epoch]
            
        else:
            num_bad_epochs += 1
        
        if verbose:
            print('Validation loss: {} Best: {}'.format(val_losses[epoch], best_loss))


    print('ChebNet training finished.')
    # Return the conditional density model that marginalizes out the grid
    return model