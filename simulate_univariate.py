from generate import gen_univ, quant_univ
...
model='linear'; error='sinex_t'; d=1; df=1; SIZE=2**8;
#model='linear'; error='t'; d=1; df=1; SIZE=2**9;
#model='linear'; error='t'; d=1; df=1; SIZE=2**10;
#model='wave'; error='expx_t'; d=1; df=2; SIZE=2**8;
#model='wave'; error='expx_t'; d=1; df=2; SIZE=2**9;
#model='wave'; error='expx_t'; d=1; df=2; SIZE=2**10;
#model='angle'; error='sinex_t'; d=1; df=2; SIZE=2**8;
#model='angle'; error='sinex_t'; d=1; df=2; SIZE=2**9;
#model='angle'; error='sinex_t'; d=1; df=2; SIZE=2**10;
...
# Prepare a dictionary to store all L1/L2 error tensors for each method
...
for r in range(R):
    ...
    quants = quant_univ(x_test, taus, model=model, error=error, df=df)
...
round_dig = 2
tau_values = np.round(taus.squeeze().numpy(), 3)  # <-- round tau values here
...
#%% Summarize the simulated results
...
summary_dir = './simulation_univariate'
...
output_excel_long = os.path.join(summary_dir, f'{model}_{error}_summary_full.xlsx')
